...
processed_dir = './data/AMASS1/amass_fps60'
...
with open('data/AMASS1/fps.csv', 'w') as f:
    print('fname_new, len_ori, fps, len_new', file=f)
    for fname in sorted(files):
        try:
            raw_x = np.load(fname)
            x = dict(raw_x)
            fps = x['mocap_framerate']  # 120
            print(fps)
            len_ori = len(x['trans'])
...
for file in *.tar.bz2; do tar -xjf "$file" && echo "解压完成: $file"; done && echo "批量解压完成！"

for file in /path/to/your/source_directory/*.tar.bz2; do tar -xjf "$file" -C /path/to/your/extract_directory && echo "解压完成: $file"; done && echo "批量解压完成！"

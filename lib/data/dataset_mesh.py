...
from lib.data.augmentation import Augmenter3D, Augmenter2D
...
class SMPLDataset(Dataset):
    def __init__(self, args, data_split, dataset): # data_split: train/test; dataset: h36m, coco, pw3d, pvcp
        random.seed(0)
        np.random.seed(0)
        ...
        if dataset=="h36m":
            ...
            datareader = DataReaderMesh(n_frames=self.clip_len, sample_stride=args.sample_stride, data_stride_train=args.data_stride, data_stride_test=self.clip_len, dt_root=args.data_root, dt_file=args.dt_file_pvcp)
        elif dataset=="pedx":
            datareader = DataReaderMesh(n_frames=self.clip_len, sample_stride=args.sample_stride, data_stride_train=args.data_stride, data_stride_test=self.clip_len, dt_root=args.data_root, dt_file=args.dt_file_pedx)
        else:
        ...
        self.motion_smpl_3d = {'train': {'pose': smpl_pose_train, 
                                         'shape': smpl_shape_train}, 
                               'test': {'pose': smpl_pose_test, 
                                        'shape': smpl_shape_test}}[data_split]
        if dataset=="pvcp":
            bbox_train = dt['train']['bbox'][split_id_train]
            bbox_test = dt['test']['bbox'][split_id_test]
            pose_class_train = dt['train']['pose_class'][split_id_train]
            pose_class_test = dt['test']['pose_class'][split_id_test]
            distance_train = dt['train']['distance'][split_id_train]
            distance_test = dt['test']['distance'][split_id_test]
            fps_train = dt['train']['fps'][split_id_train]
            fps_test = dt['test']['fps'][split_id_test]
            # img_feature_train =dt['train']['img_feature'][split_id_train]
            # img_feature_test = dt['test']['img_feature'][split_id_test]

            self.motion_sensor = { 'train':{'bbox': bbox_train, 
                                    'pose_class': pose_class_train,
                                    'distance': distance_train,
                                    'fps': fps_train,
                                    # 'img_feature':img_feature_train
                                    }, 
                                    'test': {'bbox': bbox_test, 
...
class MotionSMPL(SMPLDataset):
    ...
    def __getitem__(self, index):
        ...
        motion_sensor = []
        if self.dataset=="pvcp":
            bbox = torch.tensor(self.motion_sensor["bbox"][index], dtype=torch.float32)  
            pose_class = torch.tensor([np.argmax(pc) for pc in self.motion_sensor["pose_class"][index]], dtype=torch.float32)  
            distance = torch.tensor(self.motion_sensor["distance"][index][:,-1], dtype=torch.float32)
            fps = torch.tensor(self.motion_sensor["fps"][index], dtype=torch.float32)
            # img_feature = torch.tensor(self.motion_sensor["img_feature"][index], dtype=torch.float32)
            motion_sensor = {
                            'bbox': bbox,               # (T,4),cxcywh
                            'pose_class': pose_class,   # (T,1)
                            'distance': distance,       # (T,1)
                            'fps': fps,                 # (T,1)
                            # 'img_feature': img_feature
                            }                
        
        if self.dataset=="pedx":
            pose_class = torch.tensor([np.argmax(pc) for pc in self.motion_sensor["pose_class"][index]], dtype=torch.float32)  
            motion_sensor = {'pose_class': pose_class}  
            
        if self.dataset!="pedx" and self.dataset!="pvcp":
            pose_class = torch.tensor(np.full(motion_2d.shape[0], 1), dtype=torch.float32)
            motion_sensor = {'pose_class': pose_class}  
        ...
        return motion_2d, motion_sensor, motion_smpl_3d
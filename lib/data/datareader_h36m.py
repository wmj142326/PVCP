...
#       Only Testset HW is needed for denormalization
        ...
        split_id_train, split_id_test = self.get_split_id()          # (N, 243)
    ...
    def denormalize(self, test_data):  # 反归一化，将归一化的坐标还原为原始坐标
#       data: (N, n_frames, 51) or data: (N, n_frames, 17, 3)        
        n_clips = test_data.shape[0]  # N
        test_hw = self.get_hw()  # (N, 2) [1000. 1002.]
        data = test_data.reshape([n_clips, -1, 17, 3])  # (N, n_frames, 17, 3) 

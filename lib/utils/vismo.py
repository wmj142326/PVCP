...
def render_and_save(motion_input, save_path, keep_imgs=False, fps=25, color="#F96706#FB8D43#FDB381", with_conf=False, draw_face=False, vid_size=(1920,1080)):
    ...
    if motion.shape[1]==2 or with_conf:
        ...
        motion_world = pixel2world_vis_motion(motion, dim=3)
        motion2video_3d(motion_world, save_path=save_path, keep_imgs=keep_imgs, fps=fps, vid_size=vid_size)
...
def get_img_from_fig(fig, width, height, dpi=120):
    
    # aspect_ration = width/height
    # width, _ = fig.get_size_inches()
    # height = width / aspect_ration
    # fig.set_size_inches(width, height)
    
    # # 设置坐标轴等比例
    # ax = fig.gca()
    # ax.set_box_aspect([1, aspect_ration, 1])
    # ax.auto_scale_xyz()
...
def motion2video_3d(motion, save_path, fps=25, keep_imgs = False, vid_size=(1920,1080)):
#     motion: (17,3,N)
    ...
    for f in tqdm(range(vlen)):
        ...
        frame_vis = get_img_from_fig(fig, vid_size[0], vid_size[1])
        videowriter.append_data(frame_vis)
        plt.close()
    videowriter.close()

def motion2video_mesh(motion, save_path, fps=25, keep_imgs = False, draw_face=True, vid_size=(1920,1080)):
    videowriter = imageio.get_writer(save_path, fps=fps)
    vlen = motion.shape[-1]
    ...
    for f in tqdm(range(vlen)):
        ...
        frame_vis = get_img_from_fig(fig, vid_size[0],vid_size[1], dpi=128)
        plt.cla()
        videowriter.append_data(frame_vis)
        plt.close()
    videowriter.close()

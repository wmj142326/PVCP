...
def evaluate_mesh(results):
    ...
    for n in range(num_samples):
        pred_verts_pa.append(rigid_align(pred_verts[n], target_verts[n]))
    pred_verts_pa = np.array(pred_verts_pa)
    pa_mpve = np.mean(np.sqrt(np.square(pred_verts_pa - target_verts).sum(axis=2)), axis=1)  # (N, )
    ...
    error_dict = {
        'mpve': mpve.mean(),
        'pa_mpve': pa_mpve.mean(),
        'mpjpe_14j': mpjpe.mean(),
        'pa_mpjpe_14j': pa_mpjpe.mean(),
        'mpjpe_17j': mpjpe_17j.mean(),
        'pa_mpjpe_17j': pa_mpjpe_17j.mean(),
        }
    return error_dict

def evaluate_mesh_class(results):
    pred_verts = results['verts'].reshape(-1, 6890, 3)
    target_verts = results['verts_gt'].reshape(-1, 6890, 3)

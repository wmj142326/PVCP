...
class MLP(nn.Module): # x:[B*F J 256] --> [B*F J 256]
...
class Attention(nn.Module): # x:[B*F J 256] --> [B*F J 256]
    ...
    def forward(self, x, seqlen=1):
        B, N, C = x.shape  # [B N C]--> [B N C];  = [BF, J, 256] --> [BF, J, 256] 
        if self.mode == 'series':
            ...
            qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)  #[B, N, 256]->[B, N, 3, h, C/h]->[3, B, h, N, C/h]
            q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple), [B, h, N, C/h]
            x = self.forward_spatial(q, k, v)  # [B, h, N, C/h] -> #[B, N, C]
        else:
    ...
    def forward_spatial(self, q, k, v):  # [B, h, N, C] -> [B N C*h], 这里的B,就是之前的B*F， N就是J=17， C=32，前面的C=256=32*h
        B, _, N, C = q.shape  # [B, h, N, C]; B=BF
        attn = (q @ k.transpose(-2, -1)) * self.scale  # [B, h, N, C] @ [B, h, C, N]
        attn = attn.softmax(dim=-1)  # [B, h, N, N]
        attn = self.attn_drop(attn)

        x = attn @ v  # [B, h, N, N] @ [B, h, N, C] = [B, h, N, C]
        x = x.transpose(1,2).reshape(B, N, C*self.num_heads) #[B, h, N, C] -> [B, N, h, C]-> [B, N, h*C]
        return x
        
    def forward_temporal(self, q, k, v, seqlen=8):
        B, _, N, C = q.shape  # [BT, h, N, C]
        ...
        attn = (qt @ kt.transpose(-2, -1)) * self.scale # (B, H, N, T, T) = (B, H, N, T, C) * (B, H, N, C, T)
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = attn @ vt #(B, H, N, T, C) =  (B, H, N, T, T) *(B, H, N, T, C)
        x = x.permute(0, 3, 2, 1, 4).reshape(B, N, C*self.num_heads)  # (B, H, N, T, C) -> (B, T, N, H, C)  -> (BT, N, H*C)
...
class Block(nn.Module):  # x:[B*F J 256] --> # [B*F J 256]
...
class DSTformer(nn.Module):  # x:[B F J 3] --> # [B F J 3]
    ...
    def forward(self, x, return_rep=False):   
        B, F, J, C = x.shape  # [B F J 3] 
        x = x.reshape(-1, J, C)
        BF = x.shape[0]
        x = self.joints_embed(x)  # [B*F, J, 3] --> [B*F, J, 256]
        x = x + self.pos_embed  # [B*F, J, 256] + [1, J, 256]
        _, J, C = x.shape  # [B*F J 256]
        x = x.reshape(-1, F, J, C) + self.temp_embed[:,:F,:,:]  # [B, F, J, 256] + [1, F, 1, 256]
        x = x.reshape(BF, J, C)  # [B*F J 256]
        x = self.pos_drop(x)
        alphas = []
        for idx, (blk_st, blk_ts) in enumerate(zip(self.blocks_st, self.blocks_ts)):
            x_st = blk_st(x, F)  # [B*F J 256] --> [B*F, J, 256]
            x_ts = blk_ts(x, F)
            if self.att_fuse:
                att = self.ts_attn[idx]
                alpha = torch.cat([x_st, x_ts], dim=-1)  # [B*F J 256*2]
                BF, J = alpha.shape[:2]
                alpha = att(alpha)
                alpha = alpha.softmax(dim=-1)
                x = x_st * alpha[:,:,0:1] + x_ts * alpha[:,:,1:2]  # [B*F J 256] + [B*F J 256]
            else:
                x = (x_st + x_ts)*0.5
        x = self.norm(x)            # [B*F J 256]
        x = x.reshape(B, F, J, -1)  # [B F J 256]
        x = self.pre_logits(x)      # [B F J 256] --> [B F J 512]
        if return_rep:
            return x
        x = self.head(x)  # [B F J 512] --> [B F J 3]
        return x

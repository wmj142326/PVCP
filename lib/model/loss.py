...
def loss_acceleration(predicted, target):
    ...
    if predicted.shape[1] <= 2:
        return torch.FloatTensor(1).fill_(0.)[0].to(predicted.device)
    acceleration_predicted = predicted[:, 2:] - 2 * predicted[:, 1:-1] + predicted[:, :-2]
    acceleration_target = target[:, 2:] - 2 * target[:, 1:-1] + target[:, :-2]
    return torch.mean(torch.norm(acceleration_predicted - acceleration_target, dim=-1))
...
def loss_pose_class(preds_class, gt_class):
    criterion = nn.CrossEntropyLoss()  
  
    preds_class = preds_class.view(-1, preds_class.size(-1))  # (batch_size * T, num_classes)  
    gt_class = gt_class.view(-1).to(preds_class.device)  
    gt_class = gt_class.long()  
    loss = criterion(preds_class, gt_class)  

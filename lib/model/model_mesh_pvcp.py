...
class SMPLRegressor(nn.Module):
    def __init__(self, args, dim_rep=512, num_joints=17, hidden_dim=2048, dropout_ratio=0.):
        ...
        self.head_shape = nn.Linear(hidden_dim, param_shape_dim)
    ...
    def forward(self, feat, init_pose=None, init_shape=None, n_iter=3):
        N, T, J, C = feat.shape
        NT = N * T
        feat = feat.reshape(N, T, -1)  # (N, T, J*C)
        ...
        for i in range(n_iter):
            pred_pose = self.head_pose(feat_pose) + pred_pose  # (NT,24*6)
            pred_shape = self.head_shape(feat_shape) + pred_shape
        pred_shape = pred_shape.expand(T, N, -1).permute(1, 0, 2).reshape(NT, -1) # (NT,10)
...
class MeshRegressor(nn.Module):
    def __init__(self, args, backbone, dim_rep=512, num_joints=17, hidden_dim=2048, dropout_ratio=0.5):
        ...
        # ----------class_head----------
        self.head_class =nn.Linear(dim_rep*num_joints, 4)
        

    def forward(self, x_2d, init_pose=None, init_shape=None, n_iter=3):
        '''
        ...
        smpl_class =  F.softmax(self.head_class(feat.reshape(N,T,-1)), dim = 2)
